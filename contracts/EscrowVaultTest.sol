...
/**
 * @title EscrowVaultTest — TEST ONLY
 * @dev Identical to EscrowVault but lockFunds() uses seconds instead of days for deadlines.
 *      Deploy with _timeoutSeconds = 5 to test triggerTimeout on Sepolia without waiting.
...
contract EscrowVaultTest {
    ...
    constructor(
        ...
        uint256 _amount,
        uint256 _confirmSeconds,   // seconds (not days)
        uint256 _timeoutSeconds,   // seconds (not days)
        uint256 _threshold
    ) {
        _validateParticipants(_buyer, _seller, _mediators);
        _validateAggregateKey(_pkAggCoords);
        if (_amount == 0) revert InvalidAmount();
        if (_confirmSeconds == 0 || _timeoutSeconds == 0) revert InvalidDeadline();
        ...
        confirmDeadline = _confirmSeconds;
        timeoutDeadline = _timeoutSeconds;
    ...
    // Uses seconds directly — no * 1 days multiplier
    function lockFunds() external payable {
        ...
        confirmDeadline = block.timestamp + confirmDeadline;
        timeoutDeadline = block.timestamp + timeoutDeadline;
